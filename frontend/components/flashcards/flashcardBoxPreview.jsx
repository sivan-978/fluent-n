"use client";

import Link from "next/link";
import { EllipsisVertical, Trash2, Pencil } from "lucide-react";
import { useState, useRef, useEffect } from "react";

import DeleteFlashcardSetsModal from "@/components/flashcards/deleteFlashcardSetsModal";
import { deleteSet } from "@/lib/api"


export default function flashcardBoxPreview({ data, setItems, selectionMode, isSelected, onSelect }) {
    
    const [openMenu, setOpenMenu] = useState(false);
    const menuRef = useRef(null);
    const [deleting, setDeleting] = useState(false);
    const [deleteModalOpen, setDeleteModalOpen] = useState(false);


    // close 3 dots menu when clicking outside
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (
                menuRef.current &&
                !menuRef.current.contains(event.target)
            ) {
                setOpenMenu(false);
            }
        };

        document.addEventListener("mousedown", handleClickOutside);

        return () => {
            document.removeEventListener( "mousedown", handleClickOutside );
        };
    }, []);


    // delete one set
    async function handleDeleteSet() {
        try {
            setDeleting(true);
            await deleteSet(data.id);

            setItems((prev) =>
                prev.filter((item) => item.id !== data.id)
            );

            setOpenMenu(false);
            setDeleteModalOpen(false);

        } catch (error) {
            console.error(
                "Failed to delete set:",
                error
            );
        } finally {
            setDeleting(false);
        }
    }


    return (
        <div className="relative">
            
            {/* selection checkbox */}
            {selectionMode && (
                <button
                    type="button"
                    onClick={(event) => {
                        event.preventDefault();
                        onSelect();
                    }}
                    className={`absolute top-4 left-4 z-20 w-7 h-7 rounded-lg border-2 flex items-center justify-center ${
                        isSelected
                            ? "bg-[#e85d00] border-[#e85d00]"
                            : "bg-white border-[#965c09]"
                    }`}
                >
                    {isSelected && (
                        <span className="text-white font-bold">
                            ✓
                        </span>
                    )}
                </button>
            )}


            <Link
                href={ selectionMode ? "#" : `/flashcard-sets/${data.id}` }
                onClick={(event) => {
                    if (selectionMode) {
                        event.preventDefault();
                        onSelect();
                    }
                }}
                className={`flex flex-col max-w-[340px] w-[340px]
                    bg-[#FFF4E3] border rounded-xl overflow-hidden shadow-[0_4px_12px_rgba(121,72,25,0.25)]
                    transition-all ${
                        isSelected
                            ? "border-[#e85d00] ring-2 ring-[#e85d00]"
                            : "border-[#8B4513] hover:shadow-[0_6px_18px_rgba(121,72,25,0.35)]"
                    }`}
            >

                <div>
                    <img
                        className="w-full object-cover h-60 object-top"
                        src={data.bg}
                        alt="background img"
                    />
                </div>


                <div className="flex flex-col gap-4 px-5 py-4">

                    <div className="flex flex-col gap-2 overflow-hidden">

                        <p className="text-[#7A3E0A] font-bold text-2xl overflow-hidden overflow-ellipsis break-words">
                            {data.title}
                        </p>


                        <div className="flex gap-1.5 items-center leading-tight">

                            {data.level && (
                                <span className="text-sm border border-[#8B4513] text-[#8B4513] px-1.5 rounded-lg font-semibold">
                                    {data.level}
                                </span>
                            )}

                            <span className="text-sm border border-[#8B4513] text-[#8B4513] px-1.5 rounded-lg font-semibold">
                                {data.target_language}
                            </span>

                        </div>

                    </div>


                    <div className="flex flex-col text-sm text-[#965b09e8] font-semibold leading-tight">
                        <span> {data.cardsCount} cards </span>
                        <span> Last reviewed {data.lastView} </span>
                    </div>


                    <div className="flex flex-col gap-1">
                        <div className="flex-1 min-h-2 bg-[#D88900] rounded-full" />
                        <p className="text-[#7e4d08]"> {data.completed} completed </p>
                    </div>

                </div>

            </Link>


            {/* three dots only show outside selection mode */}
            {!selectionMode && (
                <div ref={menuRef}>

                    <button
                        type="button"
                        onClick={() => setOpenMenu(!openMenu)}
                        className="absolute top-3 right-3 z-10 p-2 rounded-full bg-white/90 text-[#965c09] hover:bg-[#fbe9d0]"
                    >
                        <EllipsisVertical className="w-5 h-5" />
                    </button>


                    {openMenu && (
                        <div className="absolute right-2 top-13 flex flex-col gap-3 px-6 py-4 bg-[#fff8f0] border border-[#965c09]/10 rounded-2xl shadow-xl z-[200]">

                            <Link
                                href={`/flashcard-sets/${data.id}/edit`}
                                className="w-full flex items-center px-4 py-2 gap-2 rounded-xl text-[#965c09] bg-[#fbe9d0] hover:bg-[#dcbc8f] transition-colors"
                            >
                                <Pencil className="w-4 h-4" />
                                <span className="font-semibold text-[15px]"> Edit set </span>

                            </Link>


                            <button
                                type="button"
                                disabled={deleting}
                                onClick={() => setDeleteModalOpen(true)}
                                className="w-full flex items-center px-4 py-2 gap-2 rounded-xl text-red-600 bg-red-100 hover:bg-red-200 transition-colors cursor-pointer"
                            >
                                <Trash2 className="w-4 h-4" />
                                <span className="font-semibold text-[15px]">
                                    {deleting ? "Deleting..." : "Delete set"}
                                </span>

                            </button>

                        </div>
                    )}

                </div>
            )}

            
            {/* confirmation deleting modal*/}
            {deleteModalOpen && (
                <DeleteFlashcardSetsModal
                    setName={data.title}
                    selectedCount={1}
                    isDeleting={deleting}
                    onCancel={() => setDeleteModalOpen(false)}
                    onConfirm={handleDeleteSet}
                />
            )}
        </div>

    )
}
